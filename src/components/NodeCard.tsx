...
// Chemin sûr sous GitHub Pages (évite new URL avec BASE_URL non absolu)
...
function Icon({
  ...
  // Certaines bases n'existent qu'en .png dans /public/icons
  const PNG_ONLY = new Set(['faction-tokens', 'faction-reputation', 'reward_azoth'])
  const BASE = import.meta.env.BASE_URL
  const b = String(base || '').trim()
  const initialExt = PNG_ONLY.has(b) ? 'png' : 'webp'
  const [src, setSrc] = React.useState<string>(`${BASE}icons/${b}.${initialExt}`)
  return (
    <img
      src={src}
      alt={alt}
      ...
      fetchPriority="low"
      onError={(e) => {
        // si le .webp échoue, on tente le .png ; sinon dernier recours stable
        if (src.endsWith('.webp')) {
          setSrc(`${BASE}icons/${b}.png`)
        } else {
          setSrc(`${BASE}icons/reward_coin.png`)
        }
      }}
    />
  )
...
export default function NodeCard({ data }: { data: any }) {
  ...
  // ======== Traduction du nom de zone via la clé (zone.key) — via contexte global ========
  const { lang, locale: locMap, t } = React.useContext(LocaleContext)

  const zoneDisplayName = React.useMemo(() => {
    const nameEN = String(zone?.name || '').trim()
    const zkey = String((zone as any)?.key || '').toLowerCase()
    if (!nameEN) return ''
    if (lang === 'en-us') return nameEN
    return (locMap?.[zkey] || locMap?.[zkey.toLowerCase()] || nameEN)
  }, [zone?.name, (zone as any)?.key, lang, locMap])
  ...
  const iconMap: Record<string, string> = {
    ...
    'Faction Story Covenant': withBase('/icons/icon_factionstory_covenant_quest.png'),
    'Faction Story Marauders': withBase('/icons/icon_factionstory_marauders_quest.png'),
    'Faction Story Syndicate': withBase('/icons/icon_factionstory_syndicate_quest.png'),
    'Mount Race': withBase('/icons/icon_mountrace_quest.png'),
    'Mount Unlock': withBase('/icons/icon_mountunlock_quest.png'),
    'Objective': withBase('/icons/icon_objective_quest.png'),
    ...
    'Artifact': withBase('/icons/icon_artifact_quest.png'),
    // 'Town Project': '/icons/icon_objective_townproject.png',
  }
  const iconSrc = iconMap[normType]

  // callback de toggle : on utilise la callback fournie par Graph si dispo
  const handleToggle = (e: React.MouseEvent) => {
    e.stopPropagation()
    if (typeof data?.onToggleComplete === 'function') {
  ...
  return (
    <div
      className={
        'node-card' +
        (isCompleted ? ' completed is-done' : '') +
        (isRepeatable ? ' repeatable' : '') +
        (isMainStory ? ' mainstory' : '')
      }
    >
      {/* overlay zébré (visible quand .is-done est présent) */}
      <div className="done-overlay" aria-hidden="true" />
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        {iconSrc && (
          <img src={iconSrc} alt={normType} className="q-icon" loading="lazy" decoding="async" fetchPriority="low" />
        )}
      ...
      <div className="meta">
        ...
        {data.required_level != null && <span className="tag"><BadgeCheck size={14}/> Req.Lvl {data.required_level}</span>}
        {data.recommended_level != null && <span className="tag"><Sword size={14}/> Sugg.Lvl {data.recommended_level}</span>}
        <span className="tag"><Map size={14}/> {zoneDisplayName || zone.name}</span>
      </div>
      {data.description && (
      ...
      {(() => {
        ...
        return (
          <div className="rewards">
            ...
            {/* ITEMS : un badge par entrée de item_rewards (cliquable si id) */}
             {itemRewards.map((it, idx) => {
              ...
              const isHousing = iid.toLowerCase().startsWith('house_housingitem')
              const href = iid ? `https://www.nw-buddy.de/${isHousing ? 'housing' : 'items'}/${iid}` : ''
              const inner = (
                <>
                   {it?.icon ? (
                     <img src={it.icon} alt="" className="reward-icon" loading="lazy" decoding="async" fetchPriority="low" />
                   ) : (
                   ...
                   {it?.qty ?
                     <b>{it.qty}×</b>
                   : null}
                   <span className="item-reward__name">{String(it?.name || 'Item')}</span>
                </>
              )
              return href ? (
                <a
                ...
                <span key={key} className={`reward item-reward ${rslug ? `rarity-${rslug}` : ''}`}>
                  {inner}
                </span>
              )
             })}
      ...
      {/* Full-width completion toggle button (English) */}
      <div style={{ display:'flex', gap:8, marginTop:8 }}>
        ...
        >
          {isCompleted ? t('ui.card.markUndone','Mark as not completed') : t('ui.card.markDone','Mark as completed')}
        </button>
      </div>
    </div>
  )
